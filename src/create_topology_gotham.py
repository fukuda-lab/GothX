...
if AUTO_CONFIGURE_ROUTERS:
    routers_nodes = get_nodes_id_by_name_regexp(server, project, re.compile("VyOS1.3.0.*", re.IGNORECASE))
    routers_config = {
        # backbone
    ...
    for router in routers_nodes:
        configure_router(server, project, router, routers_config[router.name])
...
dns = create_node(server, project, coord_cloud_snorth.x - project.grid_unit * 1,
                  coord_cloud_snorth.y - project.grid_unit * 2, DNS_template_id)
create_link(server, project, cloud_snorth["node_id"], 1, dns["node_id"], 0)
set_node_network_interfaces(server, project, dns["node_id"], "eth0", ipaddress.IPv4Interface(f"{lab_nameserver}/20"),
...
ntp = create_node(server, project, coord_cloud_snorth.x + project.grid_unit * 1,
                  coord_cloud_snorth.y - project.grid_unit * 2, NTP_template_id)
create_link(server, project, cloud_snorth["node_id"], 2, ntp["node_id"], 0)
set_node_network_interfaces(server, project, ntp["node_id"], "eth0", ipaddress.IPv4Interface(f"{NTP_CLOUD_NAME[1]}/20"),
                            "192.168.0.1", lab_nameserver)
...
mqtt_cloud_tls = create_node(server, project, coord_cloud_snorth.x + project.grid_unit * 3,
                             coord_cloud_snorth.y - project.grid_unit * 2, mqtt_broker_tls_template_id)
create_link(server, project, cloud_snorth["node_id"], 3, mqtt_cloud_tls["node_id"], 0)
set_node_network_interfaces(server, project, mqtt_cloud_tls["node_id"], "eth0",
                            ipaddress.IPv4Interface(f"{MQTT_CLOUD_TLS_NAME[1]}/20"), "192.168.0.1", lab_nameserver)

#########
# Mirai #
#########
if CREATE_MIRAI_NODES:
    if CREATE_MIRAI_NODES:
    ...
    mirai_cnc = create_node(server, project, coords_east_zone[0].x - project.grid_unit,
                            coords_east_zone[0].y + project.grid_unit * 2, Mirai_cnc_template_id)
    create_link(server, project, switches_east_zone[0]["node_id"], 1, mirai_cnc["node_id"], 0)
    set_node_network_interfaces(server, project, mirai_cnc["node_id"], "eth0",
                                ipaddress.IPv4Interface(f"{sim_config['MIRAI_CNC_IPADDR']}/24"), "192.168.33.1",
                                lab_nameserver)

    mirai_scan_listener = create_node(server, project, coords_east_zone[0].x + project.grid_unit,
                                      coords_east_zone[0].y + project.grid_unit * 2, Mirai_scan_listener_template_id)
    create_link(server, project, switches_east_zone[0]["node_id"], 2, mirai_scan_listener["node_id"], 0)
    set_node_network_interfaces(server, project, mirai_scan_listener["node_id"], "eth0",
                                ipaddress.IPv4Interface(f"{sim_config['MIRAI_REPORT_IPADDR']}/24"), "192.168.33.1",
                                lab_nameserver)

    mirai_loader = create_node(server, project, coords_east_zone[0].x - project.grid_unit,
                               coords_east_zone[0].y + project.grid_unit * 3, Mirai_loader_template_id)
    create_link(server, project, switches_east_zone[0]["node_id"], 3, mirai_loader["node_id"], 0)
    set_node_network_interfaces(server, project, mirai_loader["node_id"], "eth0",
                                ipaddress.IPv4Interface("192.168.33.12/24"), "192.168.33.1", lab_nameserver)

    mirai_wget_loader = create_node(server, project, coords_east_zone[0].x + project.grid_unit,
                                    coords_east_zone[0].y + project.grid_unit * 3, Mirai_wget_loader_template_id)
    create_link(server, project, switches_east_zone[0]["node_id"], 4, mirai_wget_loader["node_id"], 0)
    set_node_network_interfaces(server, project, mirai_wget_loader["node_id"], "eth0",
                                ipaddress.IPv4Interface(f"{sim_config['MIRAI_WGET_LOADER_IPADDR']}/24"), "192.168.33.1",
                                lab_nameserver)

    mirai_bot = create_node(server, project, coord_cloud_snorth.x + project.grid_unit * 5,
                            coord_cloud_snorth.y - project.grid_unit * 2, Mirai_bot_template_id)
    create_link(server, project, cloud_snorth["node_id"], 4, mirai_bot["node_id"], 0)
    set_node_network_interfaces(server, project, mirai_bot["node_id"], "eth0",
                                ipaddress.IPv4Interface("192.168.0.100/20"), "192.168.0.1", lab_nameserver)
...
scanner = create_node(server, project, coords_east_zone[2].x - project.grid_unit,
                      coords_east_zone[2].y + project.grid_unit * 2, scanner_template_id)
create_link(server, project, switches_east_zone[2]["node_id"], 1, scanner["node_id"], 0)
set_node_network_interfaces(server, project, scanner["node_id"], "eth0", ipaddress.IPv4Interface("192.168.35.10/24"),
                            "192.168.35.1", lab_nameserver)

amplification_coap = create_node(server, project, coords_east_zone[2].x + project.grid_unit,
                                 coords_east_zone[2].y + project.grid_unit * 2, amplification_coap_template_id)
create_link(server, project, switches_east_zone[2]["node_id"], 2, amplification_coap["node_id"], 0)
set_node_network_interfaces(server, project, amplification_coap["node_id"], "eth0",
                            ipaddress.IPv4Interface("192.168.35.11/24"), "192.168.35.1", lab_nameserver)

mqtt_attacks = create_node(server, project, coords_east_zone[2].x - project.grid_unit,
                           coords_east_zone[2].y + project.grid_unit * 3, mqtt_attacks_template_id)
create_link(server, project, switches_east_zone[2]["node_id"], 3, mqtt_attacks["node_id"], 0)
set_node_network_interfaces(server, project, mqtt_attacks["node_id"], "eth0",
                            ipaddress.IPv4Interface("192.168.35.12/24"), "192.168.35.1", lab_nameserver)

metasploit = create_node(server, project, coords_east_zone[2].x + project.grid_unit,
                         coords_east_zone[2].y + project.grid_unit * 3, metasploit_template_id)
create_link(server, project, switches_east_zone[2]["node_id"], 4, metasploit["node_id"], 0)
set_node_network_interfaces(server, project, metasploit["node_id"], "eth0", ipaddress.IPv4Interface("192.168.35.13/24"),
                            "192.168.35.1", lab_nameserver)
...
labs_clus_comb_plain = create_cluster_of_nodes(server, project, 10, coords_west_zone[3].x - project.grid_unit * 5,
                                               coords_west_zone[3].y + project.grid_unit * 2, 5,
                                               switch_template_id, combined_cycle_template_id,
                                               switches_west_zone[3]["node_id"], 1,
                                               ipaddress.IPv4Interface("192.168.20.10/24"), "192.168.20.1",
                                               lab_nameserver, 1.5)

labs_comb_cloud_plain = create_node(server, project, coord_labs_snorth.x + project.grid_unit * 1,
                                    coord_labs_snorth.y - project.grid_unit * 2, combined_cycle_cloud_template_id)
create_link(server, project, labs_snorth["node_id"], 1, labs_comb_cloud_plain["node_id"], 0)
set_node_network_interfaces(server, project, labs_comb_cloud_plain["node_id"], "eth0",
                            ipaddress.IPv4Interface("192.168.4.1/20"), "192.168.0.1", lab_nameserver)
...
labs_clus_comb_dtls = create_cluster_of_nodes(server, project, 5, coords_west_zone[3].x - project.grid_unit * 5,
                                              coords_west_zone[3].y + project.grid_unit * 7, 5,
                                              switch_template_id, combined_cycle_tls_template_id,
                                              switches_west_zone[3]["node_id"], 2,
                                              ipaddress.IPv4Interface("192.168.20.20/24"), "192.168.20.1",
                                              lab_nameserver, 1.5)

labs_comb_cloud_dtls = create_node(server, project, coord_labs_snorth.x + project.grid_unit * 1,
                                   coord_labs_snorth.y - project.grid_unit * 4, combined_cycle_cloud_template_id)
create_link(server, project, labs_snorth["node_id"], 2, labs_comb_cloud_dtls["node_id"], 0)
set_node_network_interfaces(server, project, labs_comb_cloud_dtls["node_id"], "eth0",
                            ipaddress.IPv4Interface("192.168.4.3/20"), "192.168.0.1", lab_nameserver)
...
labs_mqtt_cloud_plain = create_node(server, project, coord_labs_snorth.x - project.grid_unit * 1,
                                    coord_labs_snorth.y - project.grid_unit * 2, mqtt_broker_1_6_template_id)
create_link(server, project, labs_snorth["node_id"], 3, labs_mqtt_cloud_plain["node_id"], 0)
set_node_network_interfaces(server, project, labs_mqtt_cloud_plain["node_id"], "eth0",
                            ipaddress.IPv4Interface(f"{LABS_BROKER_PLAIN_NAME[1]}/20"), "192.168.0.1", lab_nameserver)
...
steel_clus_pred_plain = create_cluster_of_nodes(server, project, 10, coords_west_zone[2].x + project.grid_unit * 5,
                                                coords_west_zone[2].y + project.grid_unit * 2, 5,
                                                switch_template_id, predictive_maintenance_template_id,
                                                switches_west_zone[2]["node_id"], 2,
                                                ipaddress.IPv4Interface("192.168.19.20/24"), "192.168.19.1",
                                                lab_nameserver, 1.5)
...
steel_clus_cooler_tls = create_cluster_of_nodes(server, project, 5, coords_west_zone[2].x - project.grid_unit * 5,
                                                coords_west_zone[2].y + project.grid_unit * 7, 5,
                                                switch_template_id, cooler_motor_template_id,
                                                switches_west_zone[2]["node_id"], 3,
                                                ipaddress.IPv4Interface("192.168.19.30/24"), "192.168.19.1",
                                                lab_nameserver, 1.5)
...
steel_clus_pred_tls = create_cluster_of_nodes(server, project, 5, coords_west_zone[2].x + project.grid_unit * 5,
                                              coords_west_zone[2].y + project.grid_unit * 7, 5,
                                              switch_template_id, predictive_maintenance_template_id,
                                              switches_west_zone[2]["node_id"], 4,
                                              ipaddress.IPv4Interface("192.168.19.40/24"), "192.168.19.1",
                                              lab_nameserver, 1.5)
...
neigh_mqtt_cloud_plain = create_node(server, project, coord_neigh_snorth.x + project.grid_unit * 1,
                                     coord_neigh_snorth.y - project.grid_unit * 2, mqtt_broker_1_6_template_id)
create_link(server, project, neigh_snorth["node_id"], 1, neigh_mqtt_cloud_plain["node_id"], 0)
set_node_network_interfaces(server, project, neigh_mqtt_cloud_plain["node_id"], "eth0",
                            ipaddress.IPv4Interface(f"{NEIGH_BROKER_PLAIN_NAME[1]}/20"), "192.168.0.1", lab_nameserver)
...
set_node_network_interfaces(server, project, neigh_stream_cloud["node_id"], "eth0",
                            ipaddress.IPv4Interface(f"{NEIGH_STREAMSERVER_NAME[1]}/20"), "192.168.0.1", lab_nameserver)

neigh_clus_domotic_plain = create_cluster_of_nodes(server, project, 5, coords_west_zone[1].x - project.grid_unit * 5,
                                                   coords_west_zone[1].y + project.grid_unit * 2, 5,
                                                   switch_template_id, domotic_monitor_template_id,
                                                   switches_west_zone[1]["node_id"], 1,
                                                   ipaddress.IPv4Interface("192.168.18.10/24"), "192.168.18.1",
                                                   lab_nameserver, 1.5)
...
neigh_clus_ipcam = create_cluster_of_nodes(server, project, 2, coords_west_zone[1].x + project.grid_unit * 5,
                                           coords_west_zone[1].y + project.grid_unit * 2, 2,
                                           switch_template_id, ip_camera_street_template_id,
                                           switches_west_zone[1]["node_id"], 2,
                                           ipaddress.IPv4Interface("192.168.18.15/24"), "192.168.18.1", lab_nameserver,
                                           1.5)
...
neigh_clus_city = create_cluster_of_nodes(server, project, 1, coords_west_zone[1].x + project.grid_unit * 5,
                                          ...
                                          switch_template_id, city_power_template_id, switches_west_zone[1]["node_id"],
                                          4,
                                          ipaddress.IPv4Interface("192.168.18.18/24"), "192.168.18.1", lab_nameserver,
                                          1.5)
...
museum_mqtt_cloud_plain = create_node(server, project, coord_museum_snorth.x + project.grid_unit * 1,
                                      coord_museum_snorth.y - project.grid_unit * 2, mqtt_broker_1_6_template_id)
create_link(server, project, museum_snorth["node_id"], 1, museum_mqtt_cloud_plain["node_id"], 0)
set_node_network_interfaces(server, project, museum_mqtt_cloud_plain["node_id"], "eth0",
                            ipaddress.IPv4Interface(f"{MUSEUM_BROKER_PLAIN_NAME[1]}/20"), "192.168.0.1", lab_nameserver)

museum_stream_cloud = create_node(server, project, coord_museum_snorth.x - project.grid_unit * 1,
                                  coord_museum_snorth.y - project.grid_unit * 2, stream_server_template_id)
create_link(server, project, museum_snorth["node_id"], 2, museum_stream_cloud["node_id"], 0)
set_node_network_interfaces(server, project, museum_stream_cloud["node_id"], "eth0",
                            ipaddress.IPv4Interface(f"{MUSEUM_STREAMSERVER_NAME[1]}/20"), "192.168.0.1", lab_nameserver)

museum_clus_building_plain = create_cluster_of_nodes(server, project, 5, coords_west_zone[0].x - project.grid_unit * 5,
                                                     coords_west_zone[0].y + project.grid_unit * 2, 5,
                                                     switch_template_id, building_monitor_template_id,
                                                     switches_west_zone[0]["node_id"], 1,
                                                     ipaddress.IPv4Interface("192.168.17.10/24"), "192.168.17.1",
                                                     lab_nameserver, 1.5)
...
museum_clus_ipcam = create_cluster_of_nodes(server, project, 2, coords_west_zone[0].x + project.grid_unit * 5,
                                            coords_west_zone[0].y + project.grid_unit * 2, 2,
                                            switch_template_id, ip_camera_museum_template_id,
                                            switches_west_zone[0]["node_id"], 2,
                                            ipaddress.IPv4Interface("192.168.17.15/24"), "192.168.17.1", lab_nameserver,
                                            1.5)
...
museum_clus_ipconsum = create_cluster_of_nodes(server, project, 2, coords_west_zone[0].x + project.grid_unit * 5,
                                               coords_west_zone[0].y + project.grid_unit * 7, 2,
                                               switch_template_id, stream_consumer_template_id,
                                               switches_west_zone[0]["node_id"], 3,
                                               ipaddress.IPv4Interface("192.168.17.17/24"), "192.168.17.1",
                                               lab_nameserver, 1.5)
