...
def telemetry(sleep_t, sleep_t_sd, event, die_event, mqtt_topic, broker_addr, mqtt_auth, mqtt_qos, mqtt_tls, mqtt_cacert, mqtt_tls_insecure, mqtt_keepalive, payload_columns: List[int] = None,):
    ...
    while not die_event.is_set():
        if event.is_set():
            ...
            if payload_columns is not None:
                payload = payload_generation_simple(data_iter, dataset_fieldseparator, payload_columns)
