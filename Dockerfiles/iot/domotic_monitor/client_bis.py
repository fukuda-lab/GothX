...
def ping(bin_path, destination, attempts=3, wait=10) -> bool:
...
def payload_generation(
    data_iter, dataset_fieldseparator, dataset_columns, dataset_zones, dataset_units
):
    ...
    data_dict = {
        dataset_columns[i]: {"valor": data_line[i], "unidad": dataset_units[i]}
        for i in range(len(dataset_zones))
        if dataset_zones[i] is None
    }
    for zone in set(filter(None, dataset_zones)):
        idx = [i for i in range(len(dataset_zones)) if dataset_zones[i] == zone]
        data_dict[zone] = [
            {
                "listname": dataset_columns[i],
                "valor": data_line[i],
                "unidad": dataset_units[i],
            }
            for i in idx
        ]
    print(f"zones {set(filter(None, dataset_zones))}")
    print(f"dict data {data_dict}")

    return as_xml(data_dict)


def payload_generation_simple(
    data_iter, dataset_fieldseparator, columns_to_include: List[int]
):
    data_line = next(data_iter).strip().split(dataset_fieldseparator)
    columns_included = [idx for idx in columns_to_include if idx <= len(data_line)]
    if len(columns_included) < len(columns_to_include):
        print(
            f"[ payload ] removed out of range columns {set(columns_to_include) - set(columns_included)}"
        )
    return " ".join([data_line[i] for i in columns_included])
...
def telemetry(
    ...
    while not die_event.is_set():
        if event.is_set():
            if payload_columns:
                payload = payload_generation_simple(
                    data_iter, dataset_fieldseparator, payload_columns
                )
            else:
...
def main(conf):
    ...
    if conf["DATASET_COLUMNS"] != "":
        try:
            telemetry_kwargs = {
                "payload_columns": [
                    int(idx) for idx in conf["DATASET_COLUMNS"].split(",")
                ]
            }
        except ValueError as e:
            print(f"error while reading DATASET_COLUMNS environment variable: {e}")
            print('Fall back to state where DATASET_COLUMNS=""')
            telemetry_kwargs = {}
    else:
        telemetry_kwargs = {}
    print(f"{telemetry_kwargs=} {conf.keys()}")
...
if __name__ == "__main__":
    ...
    print(
        f"[  setup  ] TLS enabled: {config['TLS']}, ca cert: {config['ca_cert_file']}, TLS insecure: {config['tls_insecure']}"
    )
